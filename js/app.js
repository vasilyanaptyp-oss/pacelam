...
import { searchCities, nearestCity, haversineKm, detourKm } from './geo.js';
...
// ---------------------------------------------------------------------------------------
// Chrome: header actions and bottom navigation
// ---------------------------------------------------------------------------------------
function renderChrome() {
  ...
  const items = [
    ['#/', 'list', t('nav_feed')],
    ['#/search', 'bell', t('nav_search')],
    ['#/post', 'plus', t('nav_post'), 'nav__post'],
    ['#/my', 'box', t('nav_my')],
    ['#/me', 'user', t('nav_profile')],
  ];
  const current = route();
  for (const [href, ic, label, cls] of items) {
    const active = (href === '#/' ? current.name === 'feed' : current.name === href.slice(2));
    const a = h('a', { href, class: [cls || '', active ? 'is-active' : ''].join(' ').trim(), 'aria-current': active ? 'page' : null });
    if (cls === 'nav__post') a.append(h('span.nav__plus', null, icon('plus')), label);
...
async function render() {
  const id = ++renderId;
  const r = route();
  const screens = { feed: screenFeed, post: screenPost, p: screenDetail, my: screenMy, search: screenSearch, me: screenProfile, auth: screenAuth, onboarding: screenOnboarding, inbox: screenInbox };
...
function metricFor(p) {
  ...
  if (state.route?.from && state.route?.to) {
    const d = detourKm(state.route, from, to);
    return { detour: d, trip, big: d <= 5 ? t('on_the_way') : t('detour', { km: fmtInt(d) }), good: d <= 5, sub: t('direct', { km: fmtInt(trip) }) };
  }
  const c = myCity();
  if (c) {
    const d = haversineKm(c.lat, c.lng, from.lat, from.lng);
    return { detour: null, trip, big: d < 3 ? t('in_your_town') : t('from_you', { km: fmtInt(d) }), good: d <= 20, sub: t('direct', { km: fmtInt(trip) }) };
  }
  return { detour: null, trip, big: t('direct', { km: fmtInt(trip) }), good: false, sub: '' };
}
...
async function doTake(p) {
  ...
  try {
    ...
    toast(t('deal_done'));
    if (route().name === 'p') render(); else go(`#/p/${p.id}`);
  } catch (e) { fail(e); }
}
  ...
  const s = sheet({
    ...
    body: h('form.form', {
      onsubmit: async (e) => {
        ...
        try { await api.placeBid(p.id, a, note.value.trim()); s.close(); toast(t('bid_placed')); render(); } catch (err) { fail(err); }
      },
    },
...
function pickPlace({ title, withRadius = false, current = null }) {
  return new Promise((resolve) => {
    ...
    const rowFor = (c, sub) => h('button.cityrow', { type: 'button', onclick: () => finish({ name: c.name, lat: c.lat, lng: c.lng }) }, h('span', null, c.name), h('small', null, sub || c.country));
    const showList = (list, sub) => { clear(results); if (!list.length) results.append(h('p.muted.small', { style: { padding: '12px 0' } }, t('no_city'))); for (const c of list) results.append(rowFor(c, sub?.(c))); };
    const initial = () => {
      const recentCities = recent.map((n) => searchCities(n, 1)[0]).filter(Boolean);
      showList(recentCities.length ? recentCities : searchCities('r', 0).slice(0, 0));
      if (!recentCities.length) { clear(results); for (const c of ['Rīga', 'Daugavpils', 'Rēzekne', 'Jelgava', 'Liepāja', 'Vilnius', 'Tallinn']) { const city = searchCities(c, 1)[0]; if (city) results.append(rowFor(city)); } }
    };
    ...
    const geoBtn = h('button.btn.btn--ghost.btn--wide', {
      ...
      onclick: () => {
        ...
        navigator.geolocation.getCurrentPosition((pos) => {
          const { city, distanceKm } = nearestCity(pos.coords.latitude, pos.coords.longitude);
          finish({ name: city.name, lat: pos.coords.latitude, lng: pos.coords.longitude, near: distanceKm });
...
function routeSheet() {
  ...
  const s = sheet({
    ...
    body: h('div.stack', null, fromBtn, toBtn, h('div.field', null, h('div.field__label', null, rangeLabel), range),
      h('div.row', null,
        h('button.btn.btn--ghost', { type: 'button', onclick: () => { state.route = null; store.set('pacelam.route', null); s.close(); render(); } }, t('delete')),
        h('button.btn.btn--primary', { type: 'button', style: { flex: '1' }, onclick: () => { if (!draft.from || !draft.to) { toast(t('validation_route'), 'error'); return; } state.route = draft; store.set('pacelam.route', draft); s.close(); render(); } }, t('save')))),
  });
}
function filterSheet() {
  const f = { ...state.filter };
  const kindSeg = chips([{ value: 'all', label: t('feed_all') }, { value: 'cargo', label: t('kind_cargo') }, { value: 'truck', label: t('kind_truck') }], { value: f.kind, onChange: (v) => { f.kind = v; } });
  const modeSeg = chips([{ value: 'all', label: t('feed_all') }, { value: 'urgent', label: t('mode_urgent') }, { value: 'planned', label: t('mode_planned') }], { value: f.mode, onChange: (v) => { f.mode = v; } });
  const vt = chips(state.ref.vehicleTypes.map((v) => ({ value: v.code, label: `${v.code} ${nameOf(v)}` })), { value: f.vehicleTypes, multi: true, onChange: (v) => { f.vehicleTypes = v; } });
  const ct = chips(state.ref.cargoTypes.map((c) => ({ value: c.id, label: nameOf(c) })), { value: f.cargoTypes, multi: true, onChange: (v) => { f.cargoTypes = v; } });
  const s = sheet({
    title: t('feed_filter'),
    body: h('div.stack', null, field(t('kind_cargo') + ' / ' + t('kind_truck'), kindSeg), field(t('mode'), modeSeg), field(t('search_vehicles'), vt), field(t('search_cargo'), ct),
      h('div.row', null,
        h('button.btn.btn--ghost', { type: 'button', onclick: () => { state.filter = { kind: 'all', mode: 'all', vehicleTypes: [], cargoTypes: [] }; s.close(); render(); } }, t('feed_all')),
        h('button.btn.btn--primary', { type: 'button', style: { flex: '1' }, onclick: () => { state.filter = f; s.close(); render(); } }, t('save')))),
  });
}
...
async function screenFeed() {
  ...
  if (api.mode === 'demo') el.append(h('div.banner', null, t('demo_banner')));
  const strip = h('div.strip');
  strip.append(h('button.picker', { type: 'button', onclick: () => { if (state.session && !state.me?.profile) { requireAuth(); return; } routeSheet(); } },
  ...
  strip.append(h('button.icon-btn', { type: 'button', 'aria-label': t('feed_filter'), 'aria-pressed': filterActive() ? 'true' : 'false', class: filterActive() ? 'is-on' : '', onclick: filterSheet }, icon('list')));
  el.append(strip);
  if (!state.session) el.append(h('div.card.cta', null, h('p', null, t('feed_visitors_cta')), h('a.btn.btn--primary', { href: '#/auth' }, t('sign_in'))));
  else if (!state.route) el.append(h('p.lead', { style: { marginBottom: '14px' } }, t('feed_route_none')));

  let list = postings.filter(passesFilter).map((p) => ({ p, m: metricFor(p) }));
  let far = [];
  if (state.route) {
    list.sort((a, b) => a.m.detour - b.m.detour);
    const max = state.route.maxDetour || 60;
    far = list.filter((x) => x.m.detour > max);
    if (!state.showAll) list = list.filter((x) => x.m.detour <= max);
  }
  const cards = h('div.cards');
  if (!list.length) cards.append(emptyState(t('feed_empty'), postings.length ? t('feed_show_all', { n: far.length || postings.length }) : t('feed_empty_hint')));
  for (const x of list) cards.append(postingCard(x.p));
  el.append(cards);
  if (far.length) el.append(h('div', { style: { marginTop: '14px' } }, h('button.btn.btn--ghost.btn--wide', { type: 'button', onclick: () => { state.showAll = !state.showAll; render(); } }, state.showAll ? t('feed_show_near') : t('feed_show_all', { n: far.length }))));
  return el;
}
...
async function screenDetail(id) {
  ...
  const m = metricFor(p);
  const el = h('section.screen.stack');
  const hero = h('div.card.detail__hero', null,
    tagRow(p),
    h('h1.detail__route', null, h('span', null, p.from_name + (p.from_radius_km ? ` ${t('km_plus', { km: p.from_radius_km })}` : '')), icon('arrow'), h('span', null, p.to_name + (p.to_radius_km ? ` ${t('km_plus', { km: p.to_radius_km })}` : ''))),
  ...
  if (deal) {
    ...
    if (deal.status === 'confirmed') {
      ...
      if (!mine) row.append(h('button.btn.btn--primary.btn--big', { type: 'button', style: { flex: '1' }, onclick: async () => { try { await api.confirmDeal(deal.id); toast(t('deal_done')); render(); } catch (e) { fail(e); } } }, icon('check'), t('deal_confirm')));
      row.append(h('button.btn.btn--danger', { type: 'button', onclick: async () => { if (!(await confirmSheet(t('deal_cancel'), `${p.from_name} → ${p.to_name}`, t('deal_cancel'), true))) return; try { await api.cancelDeal(deal.id); render(); } catch (e) { fail(e); } } }, t('deal_cancel')));
      box.append(row);
      el.append(box);
    }
  }
  ...
  if (isOwner) {
    if (p.mode === 'planned' && ['open', 'pending'].includes(p.status)) {
      const section = h('div.section', null, h('div.section__title', null, h('h2', null, t('owner_bids'))));
    ...
    if (['open', 'pending'].includes(p.status)) {
      el.append(h('div.row.row--end', { style: { marginTop: '8px' } }, h('button.btn.btn--danger', { type: 'button', onclick: async () => { if (!(await confirmSheet(t('close_posting'), t('close_confirm'), t('close_posting'), true))) return; try { await api.closePosting(p.id); toast(t('closed_ok')); render(); } catch (e) { fail(e); } } }, t('close_posting'))));
    }
  }

  // ---- bidder: my bid + actions ----
  if (!isOwner && me) {
    ...
    if (myBid) {
      el.append(h('div.card.status-line', null, h('span', null, `${t('your_bid')}: `), h('b', null, fmtMoney(myBid.amount)), myBid.status === 'accepted' ? h('span.tag.tag--planned', null, t('accepted')) : null,
        myBid.status === 'active' ? h('button.btn.btn--ghost.btn--sm', { type: 'button', style: { marginLeft: 'auto' }, onclick: async () => { try { await api.withdrawBid(myBid.id); render(); } catch (e) { fail(e); } } }, t('bid_withdraw')) : null));
    }
    const actions = actionButtons(p, { big: true, myBid });
...
async function screenPost() {
  ...
  const el = h('section.screen');
  el.append(h('h1', { style: { marginBottom: '12px' } }, t('post_title')));
...
function dateChips(draft, allowRange) {
  const from = h('input.input', { type: 'date', value: draft.date_from, min: isoDate(), 'aria-label': t('date'), onchange: () => { draft.date_from = from.value; if (!draft.date_to || draft.date_to < from.value) { draft.date_to = from.value; to.value = from.value; } paint(); } });
  ...
  const paint = () => { /* quick chips repaint on change is handled by chips() */ };
  const wrap = h('div.form__section', null, h('div.form__title', null, t('when')), quick, allowRange ? h('div.grid2', null, field(t('date'), from), field(t('date_to'), to)) : field(t('date'), from));
  return wrap;
}
...
function truckForm() {
  ...
  if (!vehicles.length) form.append(h('div.card', { style: { padding: '14px' } }, h('p', null, t('add_vehicle_first')), h('a.btn.btn--primary', { href: '#/me', style: { marginTop: '10px' } }, t('add_vehicle'))));
    ...
    const vchips = chips(vehicles.map((v) => ({ value: v.id, label: `${v.type_code} ${nameOf(vtype(v.type_code)) || ''}${v.plate ? ' · ' + v.plate : ''}` })), {
    ...
    form.append(h('div.form__section', null, h('div.form__title', null, t('my_vehicle')), vchips));
  }
...
function cargoForm() {
  ...
  const modeChips = chips([{ value: 'urgent', label: t('mode_urgent') }, { value: 'planned', label: t('mode_planned') }], { value: draft.mode, name: t('mode'), onChange: (v) => { draft.mode = v; modeHint.textContent = v === 'urgent' ? t('mode_urgent_hint') : t('mode_planned_hint'); priceLabel.textContent = v === 'urgent' ? t('price_urgent_label') : t('price_instant_label'); if (v === 'urgent') { draft.date_from = isoDate(); draft.date_to = isoDate(); } dateWrap.replaceWith(dateWrap = dateChips(draft, v === 'planned')); } });
  ...
  // cargo type + dynamic fields
  const dyn = h('div.stack.stack--tight');
  const paintDyn = () => {
    clear(dyn);
    const ct = ctype(draft.cargo_type_id);
    for (const f of (ct?.fields || [])) {
      if (f.type === 'bool') dyn.append(field(labelOf(f), chips([{ value: true, label: t('field_yes') }, { value: false, label: t('field_no') }], { value: draft.fields[f.key], name: labelOf(f), onChange: (v) => { draft.fields[f.key] = v; } })));
  ...
  const ctChips = chips(state.ref.cargoTypes.map((c) => ({ value: c.id, label: nameOf(c) })), { value: draft.cargo_type_id, name: t('cargo_type'), onChange: (v) => { draft.cargo_type_id = v; draft.fields = {}; paintDyn(); } });
  paintDyn();
  ...
  // vehicle needed
  const vtSelect = h('select.select', { 'aria-label': t('vehicle_needed'), onchange: (e) => { draft.vehicle_type_code = e.target.value || null; } }, h('option', { value: '' }, t('any_vehicle')));
  for (const g of state.ref.groups) {
    ...
    for (const v of types) og.append(h('option', { value: v.code }, `${v.code} · ${nameOf(v)}`));
    vtSelect.append(og);
  }
  form.append(field(t('vehicle_needed'), vtSelect));
...
async function screenProfile() {
  ...
  if (api.mode === 'demo') foot.append(h('button.btn.btn--ghost', { type: 'button', onclick: () => { api.resetDemo(); state.me = null; state.route = null; store.set('pacelam.route', null); store.set('pacelam.lastRoute', null); go('#/'); } }, 'Reset demo'));
  el.append(foot);
  return el;
}
  ...
  const s = sheet({
    ...
    body: h('form.form', {
      onsubmit: async (e) => {
        e.preventDefault();
        try {
          await api.addVehicle({ type_code: draft.type_code, plate: draft.plate.trim() || null, tonnage_t: num(draft.tonnage_t), volume_m3: num(draft.volume_m3), length_m: num(draft.length_m), width_m: num(draft.width_m), height_m: num(draft.height_m), is_default: draft.is_default });
          if (draft.is_default && (state.me.vehicles || []).length) { const added = (await api.getMe()).vehicles.find((v) => v.is_default) ; if (added) await api.setDefaultVehicle(added.id); }
...
async function screenAuth() {
  ...
  if (api.mode === 'demo') {
    ...
    el.append(h('div.stack.stack--tight', null, api.demoUsers.map((u) => h('button.btn.btn--primary.btn--big.btn--wide', { type: 'button', onclick: async () => { await api.signInDemo(u.id); await loadMe(); go(state.after || '#/'); state.after = null; } }, t(u.key)))));
...
async function screenOnboarding() {
  ...
  const draft = { role: 'carrier', display_name: '', city: null, phone: '', company: '' };
  const form = h('form.form', { novalidate: true });
  form.append(field(t('role'), chips([{ value: 'carrier', label: t('role_carrier') }, { value: 'customer', label: t('role_customer') }], { value: draft.role, name: t('role'), onChange: (v) => { draft.role = v; } })));
